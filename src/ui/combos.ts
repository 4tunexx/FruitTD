...
const STREAKS: { n: number; title: string; color: string }[] = [
  ...
  { n: 50, title: 'GODLIKE CUT!', color: '#f8fafc' },
];
...
export class ComboFx {
  ...
  constructor() {
    ...
    this.comboMeter.innerHTML = '<span class="combo-meter__label">COMBO</span><strong class="combo-meter__value">x0</strong>';
  ...
  reset(): void {
    ...
    this.lastTier = 0;
    this.setMeter(0);
    for (const el of Object.values(this.slots)) {
      if (!el) continue;
      el.classList.remove('is-live', 'is-swap');
      el.textContent = '';
    }
  }
  ...
  onHits(n: number, combo: number): void {
    if (n <= 0) return;
    this.window = 1.7;
    this.setMeter(combo);
    const label = n > 1 ? `+${n} HITS` : 'SLICE!';
    this.show('hit', `${label} · x${combo}`, comboColor(combo), 'hit-pop');
    if (n >= 3) this.show('streak', `${n}X MULTISLICE!`, comboColor(combo), 'streak-pop');
  }

  onReslice(n: number, nx?: number, ny?: number): void {
    if (n <= 0) return;
    this.show('reslice', `RESLICE x${n}`, n >= 2 ? '#4ade80' : '#86efac', 'reslice-pop', nx, ny);
  }
  ...
  onKills(n: number): void {
    ...
    if (title && tier > this.lastTier) {
      this.lastTier = tier;
      this.show('streak', title, color, 'streak-pop');
    }
  }

  private setMeter(combo: number): void {
  ...
  private show(slot: Slot, text: string, color: string, cls: string, nx?: number, ny?: number): void {
    ...
    if (replacing) el.classList.add('is-swap');
  }
}
