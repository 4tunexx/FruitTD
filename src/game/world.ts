...
export function slotIndexAt(x: number, z: number): number {
  let best = -1;
  let bestD = 0.72;
  for (let i = 0; i < PADS.length; i++) {
    const d = Math.hypot(x - PADS[i].x, z - PADS[i].z);
    if (d < bestD) {
      bestD = d;
      best = i;
    }
...
export function slicerCost(): number {
  return 50;
}
...
export function towerStats(level: number, main: boolean): { range: number; damage: number; fireRate: number } {
  const lv = Math.min(MAX_TOWER_LEVEL, level);
  const mul = main ? 1 : 0.72;
  return {
    range: (main ? 5.2 : 3.6) + lv * 0.45,
    damage: Math.round((main ? 10 : 7) + lv * 4 * mul),
    fireRate: (main ? 1.05 : 0.85) + lv * 0.12,
  };
}
