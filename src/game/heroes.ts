...
export function heroXpToLevel(xp: number): number {
  const safe = Math.max(0, Math.floor(xp)); let low = 1; let high = MAX_HERO_LEVEL;
  while (low < high) { const mid = Math.ceil((low + high) / 2); if (heroXpForLevel(mid) <= safe) low = mid; else high = mid - 1; }
  return low;
}
...
export function heroXpProgress(xp: number): number {
  const level = heroXpToLevel(xp); if (level >= MAX_HERO_LEVEL) return 1;
  const current = heroXpForLevel(level); const next = heroXpForLevel(level + 1);
...
export function heroSlashDamage(id: HeroId, level: number, combo: number, charge: number, pointer: PointerKind): number {
  const hero = heroDef(id); let dmg = (hero.damage + Math.max(0, level - 1) * 0.55) * heroStatMultiplier(level);
...
export function heroHitRadius(id: HeroId, pointer: PointerKind): number {
  const hero = heroDef(id); let r = hero.radius;
