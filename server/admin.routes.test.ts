...
import { ADMIN_DEV_PIN, ADMIN_STEAM_ID } from './routes/admin';
...
test('admin API routes are registered and authorized', async (t) => {
  ...
  const verifyPin = await fetch(`${base}/api/admin/verify`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ pin: ADMIN_DEV_PIN }),
  });
  assert.equal(verifyPin.status, 200);
  const pinBody = await verifyPin.json();
  assert.equal(pinBody.isAdmin, true);
  assert.equal(pinBody.adminSteamId, ADMIN_STEAM_ID);
  ...
  assert.equal((await verifySteam.json()).isAdmin, true);
  ...
  assert.equal(denySave.status, 403);
